...
contract BridgeSwapBorrowerOperations is LiquityBase, Ownable, CheckContract, IStableCollBorrowerOperations {
    string constant public NAME = "BridgeSwapBorrowerOperations";
    ...
    function addColl(uint _StableCollAmount) external override {
        ...
        _adjustTrove(msg.sender, _StableCollAmount.mul(collDecimalAdjustment).div(BRIDGE_SWAP_COLLATERAL_RARIO), true);
    }
    ...
    */
    ...
    function _requireAtLeastMinNetDebt(uint _netDebt) internal pure {
        require (_netDebt >= MIN_SWAP_NET_DEBT, "StableCollBorrowerOps: Trove's net debt must be greater than minimum");
    }
    ...
    function _getStableCollAmount(uint _LUSDDebt) internal view returns (uint) {
        return _LUSDDebt.mul(BRIDGE_SWAP_COLLATERAL_RARIO).div(DECIMAL_PRECISION).div(collDecimalAdjustment);
    }
}